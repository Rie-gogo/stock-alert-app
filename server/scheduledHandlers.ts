...
export async function dailySimulationHandler(req: Request, res: Response) {
  try {
    ...
    console.log(`[daily-simulation] Running simulation for ${dateStr}`);
    ...
    // シミュレーション実行
    const simResult = generateDailySimReport(dateStr, rsiUpper, rsiLower, stopLossPercent);
...
以下の${dateStr}のシミュレーション結果を分析し、改善提案を日本語で200文字以内で簡潔にまとめてください。
...
直近7日間のシミュレーション結果を分析し、パラメータ改善案をJSON形式で返してください。
...
以下のJSON形式で回答してください:
{
  ...
  "reason": "<改善理由を100文字以内で>"
    ...
    await notifyOwner({
      title: `📊 ${dateStr} デイリーシミュレーション完了`,
      content: `本日の結果: 損益 ${profitStr} | 勝率 ${winRateStr}\n${aiSummary ?? ""}`,
    });
