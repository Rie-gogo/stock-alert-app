...
export async function generateRealDailyReport(
  ...
  for (const stock of SIMULATION_STOCKS) {
    ...
    const result = simulateStockReal(
      ...
      rangeBoundDay,
      1.0,
      { shortStopLossPercent: SHORT_STOP_LOSS_PERCENT, lunchExitAllMinute: LUNCH_EXIT_ALL_MINUTE } // ショート専用損切り幅（0.55%）・昕休み前11:20全決済を適用
    );
