...
export function simulateStockReal(
  ...
  for (let i = 1; i < candles.length; i++) {
    ...
    const isPullbackShort = stockTrendDownForShort && curr.rsi >= SHORT_RSI_MIN && nearMA25ForShort && curr.close <= curr.ma25;
    // 戻り売り（厳選） または RSI買われすぎ+BB上限の明確な反転サイン。デッドクロス単独は採用しない。
    const shouldEnterShort = regimeAllowShort && !isStrongUp && volConfirmed &&
      !suppressEntryByHour &&
      tradeCount < MAX_TRADES_PER_DAY &&
      (isPullbackShort || (isRsiOverbought && isBbUpper));
