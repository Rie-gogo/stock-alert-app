...
- [ ] 直近レポートの取引履歴・シグナルをDBから抽出し、損益を銘柄/時間帯/シグナル種別/レジーム別に集計
- [ ] 損失パターンを特定（どの銘柄・どの場面・どのシグナルで負けているか）し、15,000円/日に届かない要因を定量化
- [ ] 診断結果に基づく具体的な戦略修正を設計・実装
- [ ] 複数営業日でバックテスト再検証（15,000円/日への接近を確認）、テスト更新
- [ ] チェックポイント保存・結果報告
