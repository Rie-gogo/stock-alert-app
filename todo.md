...
- [ ] AIアドバイザー（AIAdvisorPanel）が板情報・歩み値をどう使っているか調査
- [ ] ルールベース診断（diagnoseMarket）が板情報・歩み値をどう使っているか調査
- [ ] 売買シグナル判定（getStockChart）が板情報・歩み値を使っていないことを確認
- [ ] 架空の板情報・歩み値が売買判断に影響しないよう必要に応じて修正
- [ ] Yahoo Finance の出来高データを活用した「リアル出来高」表示を追加検討
- [ ] Heartbeat cron を JST 15:30 → JST 16:00 に変更（cron: 0 0 7 * * 1-5）
- [ ] 古いエージェントcronも同期して16:00に変更
- [ ] テスト・チェックポイント保存

## Phase 17: 架空データ完全排除 ＆ レポート時刻変更
- [ ] ウィークリーアルゴリズム改善ロジック（金曜日のパラメータ自動調整）が実データのみを使っていることを確認
- [ ] ホームページUI上の「板情報」「歩み値」ブロックを削除
- [ ] 代わりに「実出来高分析パネル」を追加（Yahoo Finance のローソク足出来高を可視化）
- [ ] AIシグナル判定（getStockChart）が板情報・歩み値を使っていないことを再確認
- [ ] Heartbeat cron を JST 16:00 に変更（cron: 0 0 7 * * 1-5）
- [ ] 古いエージェントcronも同期して16:00に変更
- [ ] AIAdvisorPanelのテスト追加
- [ ] チェックポイント保存
