...
- [ ] AIAdvisorPanel.tsx を常時自動実行（30秒ごと自動更新）に変更
- [ ] 自動実行中であることを示すUI（ローディングインジケーター・最終更新時刻）を追加
- [ ] simulation.ts に空売り（ショート）ロジックを追加（デッドクロス時に空売りエントリー、ゴールデンクロス時に買い戻し）
- [ ] realSimulation.ts にも空売りロジックを反映
- [ ] レポートの取引履歴に「空売り」「買い戻し」の種別を表示
- [ ] テスト更新・チェックポイント保存
