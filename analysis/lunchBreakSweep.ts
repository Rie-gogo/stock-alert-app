/**
 * lunchBreakSweep.ts
 * 昼休み前のポジション手仕まい戦略をスイープ検証する。
 *
 * 検証パターン:
 * - Baseline: 変更なし（昼休み前手仕まいなし）
 * - 11:25前にロングを手仕まい
 * - 11:20前にロングを手仕まい
 * - 11:15前にロングを手仕まい
 * - 11:25前にロング+ショートを手仕まい
 * - 11:20前にロング+ショートを手仕まい
 *
 * 実行: cd /home/ubuntu/stock-alert-app && npx tsx analysis/lunchBreakSweep.ts
 */
...
const patterns: SweepPattern[] = [
  { label: "Baseline (変更なし)", overrides: { shortStopLossPercent: SHORT_STOP_LOSS_PERCENT } },
  { label: "昼休み前11:25ロング手仕まい", overrides: { shortStopLossPercent: SHORT_STOP_LOSS_PERCENT, lunchExitLongMinute: "11:25" } },
  { label: "昼休み前11:20ロング手仕まい", overrides: { shortStopLossPercent: SHORT_STOP_LOSS_PERCENT, lunchExitLongMinute: "11:20" } },
  { label: "昼休み前11:15ロング手仕まい", overrides: { shortStopLossPercent: SHORT_STOP_LOSS_PERCENT, lunchExitLongMinute: "11:15" } },
  { label: "昼休み前11:25全手仕まい", overrides: { shortStopLossPercent: SHORT_STOP_LOSS_PERCENT, lunchExitAllMinute: "11:25" } },
  { label: "昼休み前11:20全手仕まい", overrides: { shortStopLossPercent: SHORT_STOP_LOSS_PERCENT, lunchExitAllMinute: "11:20" } },
];
...
function main() {
  ...
  console.log(`[lunchSweep] Days: ${allDays.length}`);
  ...
  for (const pattern of patterns) {
    ...
    for (const day of allDays) {
      ...
      for (const s of TARGET_STOCKS) {
        ...
        const res = simulateStockReal(s.symbol, s.ticker, s.name, candles, ctx.marketBiasAt, 3_000_000, 70, 30, 2.0, ctx.rangeBound, 1.0, pattern.overrides);
        if (!res) continue;
    ...
    results.push({ label: pattern.label, totalProfit: Math.round(grandTotal), avgPerDay: Math.round(grandTotal / tradedDays), medianPerDay: Math.round(median), winRate, posDays, negDays, worstDay: Math.round(worstDay), daysOver15k });
    console.log(`[lunchSweep] ${pattern.label}: total=${Math.round(grandTotal).toLocaleString()} avg=${Math.round(grandTotal/tradedDays)} worst=${Math.round(worstDay)}`);
  }

  const best = results.reduce((a, b) => a.totalProfit > b.totalProfit ? a : b);
  console.log(`\n====== 昼休み前手仕まいスイープ結果 ======`);
  console.log(`${"パターン".padEnd(30)} ${"総損益".padStart(10)} ${"日平均".padStart(8)} ${"中央値".padStart(8)} ${"勝率".padStart(7)} ${"最悪日".padStart(10)} ${"15k+".padStart(6)}`);
  console.log("-".repeat(85));
  for (const r of results) {
    const marker = r.label === best.label ? " ★" : "";
    console.log(`${r.label.padEnd(30)} ${r.totalProfit.toLocaleString().padStart(10)} ${r.avgPerDay.toString().padStart(8)} ${r.medianPerDay.toString().padStart(8)} ${(r.winRate*100).toFixed(1).padStart(6)}% ${r.worstDay.toLocaleString().padStart(10)} ${r.daysOver15k.toString().padStart(6)}${marker}`);
  }
  console.log(`\n[lunchSweep] Best: ${best.label} → ${best.totalProfit.toLocaleString()}円`);
}
