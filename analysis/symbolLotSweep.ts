/**
 * symbolLotSweep.ts
 * 損失超過銘柄（7011・8306・6758）を極小ロット（LOT_SMALL）に変更した場合の効果を検証する
 * jq_backtest.ts と同一のデータ処理ロジックを使用する
 *
 * 実行: cd /home/ubuntu/stock-alert-app && npx tsx analysis/symbolLotSweep.ts
 */
...
const LOT_SMALL = REGIME_CONSTANTS.LOT_SMALL;

// テストするパターン: 追加でLOT_SMALL扱いにする銘柄の組み合わせ
const PATTERNS: Array<{ name: string; extraSmall: Set<string> }> = [
  { name: "Baseline (変更なし)", extraSmall: new Set() },
  { name: "7011のみ極小ロット", extraSmall: new Set(["7011"]) },
  { name: "8306のみ極小ロット", extraSmall: new Set(["8306"]) },
  { name: "6758のみ極小ロット", extraSmall: new Set(["6758"]) },
  { name: "7011+8306 極小ロット", extraSmall: new Set(["7011", "8306"]) },
  { name: "7011+6758 極小ロット", extraSmall: new Set(["7011", "6758"]) },
  { name: "8306+6758 極小ロット", extraSmall: new Set(["8306", "6758"]) },
  { name: "7011+8306+6758 全て極小ロット", extraSmall: new Set(["7011", "8306", "6758"]) },
];
...
function main() {
  ...
  console.log(`[symbolLotSweep] Days: ${allDays.length}`);

  const results: Array<{ name: string; total: number; avg: number; median: number; worst: number; best: number; days15k: number; posDays: number }> = [];
  ...
  for (const pattern of PATTERNS) {
    ...
    for (const day of allDays) {
      ...
      for (const s of TARGET_STOCKS) {
        ...
        // 追加極小ロット銘柄かどうかでlotRatioを決定
        const lotRatioOverride = pattern.extraSmall.has(s.symbol) ? LOT_SMALL : undefined;
        const res = simulateStockReal(
          s.symbol, s.ticker, s.name, candles, marketBiasByProgress, 3_000_000, 70, 30, 2.0, rangeBound, 1.0,
          { shortStopLossPercent: SHORT_STOP_LOSS_PERCENT, lunchExitAllMinute: LUNCH_EXIT_ALL_MINUTE, lotRatio: lotRatioOverride }
        );
        if (!res) continue;
    ...
    const avg = total / dailyProfits.length;
    const median = sorted[Math.floor(sorted.length / 2)];
    const worst = sorted[0];
    const best = sorted[sorted.length - 1];
    const days15k = dailyProfits.filter(p => p >= 15000).length;
    const posDays = dailyProfits.filter(p => p > 0).length;
    results.push({ name: pattern.name, total, avg, median, worst, best, days15k, posDays });
    console.log(`[symbolLotSweep] ${pattern.name}: total=${total.toLocaleString()} avg=${Math.round(avg)} worst=${worst.toLocaleString()}`);
  }

  console.log("\n====== 銘柄ロットスイープ結果 ======");
  console.log(`${"パターン".padEnd(32)} ${"総損益".padStart(10)} ${"日平均".padStart(8)} ${"中央値".padStart(8)} ${"最悪日".padStart(10)} ${"15k+日".padStart(6)} ${"プラス日".padStart(6)}`);
  console.log("-".repeat(90));
  const bestTotal = Math.max(...results.map(r => r.total));
  for (const r of results) {
    const mark = r.total === bestTotal ? " ★" : "";
    console.log(
      `${r.name.padEnd(32)} ${r.total.toLocaleString().padStart(10)} ${Math.round(r.avg).toLocaleString().padStart(8)} ${r.median.toLocaleString().padStart(8)} ${r.worst.toLocaleString().padStart(10)} ${r.days15k.toString().padStart(6)} ${r.posDays.toString().padStart(6)}${mark}`
    );
  }
  const best = results.reduce((a, b) => a.total > b.total ? a : b);
  console.log(`\n[symbolLotSweep] Best: ${best.name} → ${best.total.toLocaleString()}円`);
}
