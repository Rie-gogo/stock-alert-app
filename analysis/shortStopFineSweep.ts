/**
 * shortStopFineSweep.ts
 * ショート損切り幅 0.3%〜0.9%（0.05%刻み）の細かいスイープ。
 * J-Quants 60営業日データを使用。
 *
 * 実行: cd /home/ubuntu/stock-alert-app && npx tsx analysis/shortStopFineSweep.ts
 */
...
function main() {
  ...
  console.log(`[fineSweep] Days: ${allDays.length}`);
  ...
  // 0.3%〜0.9%を0.05%刻みでスイープ
  const stopLevels: number[] = [];
  for (let s = 0.30; s <= 0.90; s += 0.05) stopLevels.push(Math.round(s * 100) / 100);
  ...
  for (const stop of stopLevels) {
    ...
    results.push({ stop, totalProfit: Math.round(grandTotal), avgPerDay: Math.round(grandTotal / tradedDays), medianPerDay: Math.round(median), winRate, posDays, negDays, worstDay: Math.round(worstDay), daysOver15k, totalTrades });
    console.log(`[fineSweep] stop=${stop.toFixed(2)}% total=${Math.round(grandTotal).toString().padStart(9)} avg=${Math.round(grandTotal/tradedDays).toString().padStart(7)} median=${Math.round(median).toString().padStart(8)} wr=${(winRate*100).toFixed(1)}% pos=${posDays} neg=${negDays} worst=${Math.round(worstDay).toString().padStart(9)} 15k=${daysOver15k} trades=${totalTrades}`);
  }

  const best = results.reduce((a, b) => a.totalProfit > b.totalProfit ? a : b);
  console.log(`\n====== 細かいスイープ結果（総損益順）======`);
  const sorted2 = [...results].sort((a, b) => b.totalProfit - a.totalProfit);
  console.log(`${"損切り幅".padEnd(8)} ${"総損益".padStart(10)} ${"日平均".padStart(8)} ${"中央値".padStart(8)} ${"勝率".padStart(7)} ${"最悪日".padStart(10)} ${"15k+日".padStart(7)} ${"取引数".padStart(7)}`);
  console.log("-".repeat(75));
  for (const r of sorted2) {
    const marker = r.stop === best.stop ? " ★最良" : "";
    console.log(`${(r.stop.toFixed(2) + "%").padEnd(8)} ${r.totalProfit.toString().padStart(10)} ${r.avgPerDay.toString().padStart(8)} ${r.medianPerDay.toString().padStart(8)} ${(r.winRate*100).toFixed(1).padStart(6)}% ${r.worstDay.toString().padStart(10)} ${r.daysOver15k.toString().padStart(7)} ${r.totalTrades.toString().padStart(7)}${marker}`);
  }

  const outDir = path.join(process.cwd(), "analysis", "jq_out");
  fs.mkdirSync(outDir, { recursive: true });
  const csvRows = ["stopPercent,totalProfit,avgPerDay,medianPerDay,winRate,posDays,negDays,worstDay,daysOver15k,totalTrades"];
  for (const r of results) {
    csvRows.push([r.stop, r.totalProfit, r.avgPerDay, r.medianPerDay, r.winRate.toFixed(4), r.posDays, r.negDays, r.worstDay, r.daysOver15k, r.totalTrades].join(","));
  }
  fs.writeFileSync(path.join(outDir, "short_stop_fine_sweep.csv"), csvRows.join("\n"), "utf8");
  console.log(`\n[fineSweep] Best: ${best.stop}% → Total: ${best.totalProfit.toLocaleString()}円`);
}
